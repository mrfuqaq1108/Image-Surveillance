...
def diff_f_avg(delta_f):
    M = 30
    C = 0.5  # datasize 0.5MB
    C_delta = 2000  # CPU cycles  2000Megacycles
    epsilon = 10 ** (-11)
    f = np.linspace(1.1, 1.2, M)+delta_f  # local computing frequency (GHz)
...
x = np.linspace(0.55, 2.35, 19)

import matplotlib.pyplot as plt
fig = plt.figure()
font_xy = {'family': 'Times New Roman', 'size': '15'}
font1 = {'family': 'Times New Roman', 'weight': 'bold', 'size': '15'}
plt.xlabel("Average CPU frequency of cameras (GHz)", font1, labelpad=10)
plt.ylabel('Average task value', font1, labelpad=10)

plt.xlim(0.55, 2.35)
plt.ylim(2, 4.4)

plt.plot(x, our_proposed, color='r', marker='s', markersize=8, label="our proposed", linewidth=2)
plt.plot(x, local, color='g', marker='^', markersize=8, label="local computing", linewidth=2)
plt.plot(x, edge, color='c', marker='o', markersize=8, label="edge computing", linewidth=2)

plt.xticks(np.arange(0.55, 2.36, 0.2), family='Times New Roman', fontsize=15)
plt.yticks(np.arange(2, 4.5, 0.4), family='Times New Roman', fontsize=15)
...
# plt.savefig("D:/python_code/access/results/compare_f.pdf")
